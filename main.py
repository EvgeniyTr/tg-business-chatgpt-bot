...
class BotManager:
    ...
    async def _initialize(self):
        ...
        self.application.add_handler(ChatJoinRequestHandler(self._handle_chat_join))
    ...
    async def _set_default_commands(self):
        commands = [
            ...
            BotCommand("generate_image", "Сгенерировать изображение")
        ]
    ...
    async def _handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            logger.info(f"Новое сообщение от {user.id} в чате {chat.id} ({chat.type}): {message.text}")
            ...
            if chat.type in ["group", "supergroup"]:
                ...
                if not (message.text.startswith('@') and context.bot.username in message.text):
                    return
    ...
    async def _start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            f"Привет, {user.first_name}! Я твой цифровой ассистент. "
            "Могу отвечать на сообщения и генерировать изображения."
        )

    async def _handle_chat_join(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Обработчик вступления в чат"""
        chat = update.effective_chat
        user = update.effective_user
        logger.info(f"Бот вступил в чат {chat.id} ({chat.title})")
        
        # Отправляем приветственное сообщение
        await context.bot.send_message(
            chat_id=chat.id,
            text="Спасибо за добавление! Я буду помогать с ответами и генерацией контента. "
    ...
    async def _setup_webhook(self):
        ...
        await self.application.bot.set_webhook(
            url=webhook_url,
            allowed_updates=["message", "voice", "chat_join_request"]
        )
